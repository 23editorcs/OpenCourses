...
(check-expect (contains? (cons "UBC" (cons "McGill" empty)) "Quy") false)
(check-expect (contains? (cons "UBC" (cons "McGill" empty)) "McGill") true)
...
(define (squares lon) (butter sqr lon))
...
(define (square-roots lon) (butter sqrt lon))

(define (butter fn lon)
...
(define (positive-only lon) (peanut positive? lon))
...
(define (negative-only lon) (peanut negative? lon))

(define (peanut fn lon)
  (cond [(empty? lon) empty]
        [else
         (if (fn (first lon))
             (cons (first lon)
